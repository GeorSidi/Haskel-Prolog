...
twice(C,N) :- hosted(I1,Y1,C) , I1 \==I2 ,
			  hosted(I2,Y2,C) , (Y2 - Y1) >= N,!.

in2Cities(D) :- country(C1,D) , country(C2,D) ,
				hosted(I1,Y1,C1) , C1 \== C2 ,
				hosted(I2,Y2,C2),! . 

in3Continents(I) :- continent(D1,E1) ,continent(D2,E2),
					continent(D3,E3) ,
					E1\==E2 , E2\==E3 ,E3\==E1,
					country(C1,D1),country(C2,D2),country(C3,D3),
					D1\==D2 ,D2\==D3 ,D3\==D1 ,
					hosted(I,Y1,C1) ,hosted(II,Y2,C2),
					hosted(III,Y3,C3) , 
					II is I+1,
					III is I+2,!.
...
move(A,B,C) :- tsouf(A,B,C),!.
tsouf(A,B,C):- nextStation(LN1,A,D1),
				entranceFare(LN1,E),
				nextStation(LN2,D,D1),LN1 = LN2,
				travelFare(LN1,E1), CN is E+E1 ,CN<C. 
tsouf(A,B,C):- nextStation(LN1,A,D),
				entranceFare(LN1,E),
				nextStation(LN2,D,D1),LN1 \= LN2,
				entranceFare(LN2,E2), CN is E+E2 ,CN<C. 
...
p(N) :-  I is N div 2 , h(N,I),!.

h(N,1).
h(N,K):- not(not_p(N,K)) ,C is K-1 , h(N,C) .

prime(N) :- N mod 3 =\= 0 , N mod 2 =\= 0 ,N mod 5 =\= 0.
prime(2):- true.
prime(3):- true.
prime(5):- true.

not_p(N,L):- prime(L) , L1 is L*L , N mod L1 =:= 0.
